...
Text: ${attachment.text}
  `
        ...
        const formattedSwappedPostExamples = this.character.swappedPostExamples
            ?.sort(() => 0.5 - Math.random())
            .map((example) => `${example}`)
            .slice(0, 5)
            .join("\n")
        ...
        const initialState = {
            ...
            allianceIntents:
                ...
                formattedAllianceIntents.replaceAll("\n", "").length > 0
                    ...
                    : "",
            swappedPostExamples:
                formattedSwappedPostExamples &&
                formattedSwappedPostExamples.replaceAll("\n", "").length > 0
                    ? addHeader(
                        `# Swapped Post Examples for ${this.character.name}`,
                        formattedSwappedPostExamples
                    )
                    : "",
